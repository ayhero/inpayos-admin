...
export function MerchantRouter() {
  ...
  const fetchRouters = async () => {
    ...
    try {
      ...
      const response = await routerService.listMerchantRouters(params);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await routerService.deleteMerchantRouter(routerToDelete.id);
      if (response.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = selectedRouter
        ? await routerService.updateMerchantRouter({
            id: selectedRouter.id,
            ...formData
          })
        : await routerService.createMerchantRouter(formData);
  ...
  useEffect(() => {
    setCurrentPage(1);
    fetchRouters();
  }, [searchParams.user_id, searchParams.trx_type, searchParams.channel_code, searchParams.status]);
                ...
                {(routerToDelete?.ccy || routerToDelete?.min_amount || routerToDelete?.max_amount) && (
                  <div className="flex gap-2">
                    ...
                    <span className="text-gray-900">
                      {routerToDelete?.ccy ? getCcyLabel(routerToDelete.ccy) : '-'} 
                      {' '}
                      {routerToDelete?.min_amount || 0} ~ {
